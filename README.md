...
- Lombok [version:1.18.38] (the java library for removing boiler plate code from pojos)
- Spring-Boot [version:3.3.13] (the framework for creating spring application that just run)
- Spring-Data [version:3.3.13] (the framework for interacting with database)
- H2 Database [version:2.2.224] (the database we use for storing the information in development enviroment)
- Liquibase [version:4.27.0] (the tool for keeping the version control for relational databases)
- Jakarta XML Web Services / JAX-WS RI [version:4.0.3] (the library for exposing soap web service endpoints)
