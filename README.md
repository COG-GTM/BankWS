...
- Maven [version:3.9] (the tool for managing dependencies and building the project) 
- Lombok [version:1.18.34] (the java library for removing boiler plate code from pojos)
- Spring-Boot [version:3.2.5] (the framework for creating spring application that just run)
- Spring-Data [managed by Spring Boot 3.2.5] (the framework for interacting with database)
- H2 Database [version:2.x, managed by Spring Boot] (the database we use for storing the information in development enviroment)
- Liquibase [version:4.x, managed by Spring Boot] (the tool for keeping the version control for relational databases)
- JAX-WS [version:4.0.3] (the library for exposing soap web service endpoints)
